...
def generate_log(username=None):
    ...
    for user in users:
        ...
        log_lines.append(f"Number of Agents: {len(user.agents_data) if user.agents_data else 0}")
        if user.agents_data:
            for agent in user.agents_data:
                ...
                log_lines.append(f"Agent Relationships: {agent.get('relationships', [])}")
                log_lines.append(f"Agent Persona: {agent.get('persona', '')}")
                log_lines.append(f"Agent Summary: {agent.get('summary', '')}")
                log_lines.append(f"Agent Keywords: {', '.join(agent.get('keywords', []))}")
                log_lines.append(f"Agent Image Prompt: {agent.get('image_prompt', '')}")
                log_lines.append("")
        ...
        for meeting in meetings:
            if meeting.id not in meeting_ids:
                ...
                log_lines.append(f"Number of Agents: {len(meeting.agents) if meeting.agents else 0}")
                log_lines.append(f"Number of Questions: {len(meeting.questions) if meeting.questions else 0}")
                log_lines.append(f"Number of Answers: {len(meeting.answers) if meeting.answers else 0}")
                log_lines.append("")

        log_lines.append(f"Number of Images: {len(user.images_data) if user.images_data else 0}")
        log_lines.append(f"Number of Thumbnails: {len(user.thumbnail_images_data) if user.thumbnail_images_data else 0}")
        log_lines.append("\n")
