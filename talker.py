...
def talker(agent_id):
    ...
    try:
        ...
        return render_template("talker.html", agent=agent, agent_image_data=agent_image_data, agent_id=agent_id)
...
def transcribe():
    ...
    try:
        with open(audio_file_path, 'rb') as f:
            transcription = client.audio.transcriptions.create(model="whisper-1", file=f, language="en")
...
def chat_with_model(user_message, agent, max_tokens=250, top_p=0.5, temperature=0.9):
    system_prompt = f"{agent.id} is {agent.data.get('jobtitle', '')}. {agent.data.get('summary', '')}\n\nPersona:\n{agent.data.get('persona', '')}\n\nRelationships:\n{agent.data.get('relationships', '')}"
...
def text_to_speech():
    ...
    user_folder = current_user.folder_path
    os.makedirs(user_folder, exist_ok=True)
    audio_file_path = os.path.join(user_folder, 'response.mp3')
