...
def test_upload_document():
    ...
    try:
        ...
        response = requests.post(
            url_for('doc2api_blueprint.upload_document', _external=True),
        ...
        if response.status_code == 200:
            ...
            logging.error(f"upload_document endpoint returned status code {response.status_code}")
            logging.error(f"Response data: {response.json()}")

    except requests.exceptions.RequestException as e:
        logging.error(f"Error occurred while testing upload_document endpoint: {str(e)}")
...
def test_edit_document():
    ...
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {'message': 'Document updated successfully'}
    ...
    try:
        ...
        response = requests.post(
            url_for('doc2api_blueprint.edit_document', document_id=test_document_id, _external=True),
        ...
        if response.status_code == 200:
            ...
            logging.error(f"edit_document endpoint returned status code {response.status_code}")
            logging.error(f"Response data: {response.json()}")

    except requests.exceptions.RequestException as e:
        logging.error(f"Error occurred while testing edit_document endpoint: {str(e)}")
...
def test_submit_document():
    ...
    test_document_id = 1
    test_document_data = {
        'agents': [{'name': 'Test Agent', 'jobtitle': 'Test Job', 'description': 'Test Description'}],
        'timeframes': [{'name': 'Test Timeframe'}],
        'meetings': [{'name': 'Test Meeting'}]
    }

    # Mock the API request to submit the document
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {'message': 'Document submitted successfully'}
    ...
    try:
        ...
        response = requests.post(
            url_for('doc2api_blueprint.submit_document', document_id=test_document_id, _external=True),
            json=test_document_data
        )
        ...
        if response.status_code == 200:
            ...
            logging.error(f"submit_document endpoint returned status code {response.status_code}")
            logging.error(f"Response data: {response.json()}")

    except requests.exceptions.RequestException as e:
        logging.error(f"Error occurred while testing submit_document endpoint: {str(e)}")
