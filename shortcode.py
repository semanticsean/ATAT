...
def handle_document_short_code(email_content, api_key, conversation_history=None):
    ...
    if match:
        ...
        logging.info(f"Style shortcode: {structured_response}, {new_email_content}")
    ...
    if detail_matches:
        ...
        for match in detail_matches:
            ...
            chunks = split_content_into_chunks(detail_content)
            detailed_responses.extend(chunks)

        new_email_content = re.sub(r"!detail\(.*?\)", "", email_content, flags=re.DOTALL).strip()

        logging.info(f"Detail shortcode matched: {detailed_responses}, {new_email_content}")

        return json.dumps({'type': 'detail'}), detailed_responses, new_email_content
