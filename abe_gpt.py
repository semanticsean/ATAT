...
def process_agents(payload, current_user):
    ...
    new_timeframe = Timeframe(name=payload["timeframe_name"],
                              ...
                              agents_data=[])  # Initialize agents_data as an empty list
    ...
    for agent in agents_data:
        ...
        new_timeframe.agents_data.append({
            'agent_id': agent['id'],
        ...
        while retry_count < max_retries:
            try:
                dalle_response = client.images.generate(
                    model="dall-e-3",
                    prompt=dalle_prompt[:5000],  # Truncate the prompt to the maximum allowed length
                    quality="standard",
                    size="1024x1024",
                    n=1,
                )
                ...
                logging.error(f"OpenAI API error: {e}")
                logging.error(f"DALL-E prompt: {dalle_prompt}")  # Log the prompt for debugging
                retry_count += 1
                if retry_count < max_retries:
                    time.sleep(retry_delay)
                    retry_delay *= 2
        ...
        if dalle_response is None:
            ...
            try:
                ...
                new_timeframe.images_data[f"timeframe_{new_timeframe.id}_{new_photo_filename}"] = encoded_string
                ...
                new_timeframe.thumbnail_images_data[f"timeframe_{new_timeframe.id}_{new_photo_filename}"] = thumbnail_encoded_string
