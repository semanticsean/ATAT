...
# Set up a single log file for all logging
log_file_path = "abe_gpt.log"
...
def summarize_process_agents(new_timeframe, payload, current_user):
    try: 
        ...
        logging.info("Preparing image prompt for DALL-E")
        ...
        try:
            for agent in agents_data:
                ...
                job_title = agent.get('jobtitle', 'Unknown Job Title')  # Provide default value if jobtitle is missing
                summary = agent.get('summary', 'No summary provided')  # Provide default value if summary is missing
                image_prompt = agent.get('image_prompt', 'No image prompt provided')  # Provide default value if image_prompt is missing
                image_prompt += f"ID: {agent_id}, Job Title: {job_title}, Summary: {summary}, Image Prompt: {image_prompt}\n"
        except KeyError as e:
            logging.error(f"Missing key in agent data: {str(e)}")
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                logging.error(f"OpenAI API error during image generation: {str(e)}")
                logging.error(f"Error details: {e.error}")  # Log the error details
                retry_count += 1
                if retry_count < max_retries:
        ...
        logging.error(f"An error occurred in summarize_process_agents: {str(e)}")
...
def process_meeting_summary(meeting, current_user):
    ...
    with open("abe/abe-instructions.json", "r") as file:
        abe_instructions = json.load(file)
        meeting_summary_instructions = abe_instructions.get("meeting_summary_instructions", "")
    ...
    # Truncate the image_prompt to a maximum of 5,000 characters
