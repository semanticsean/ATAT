...
def check_smtp_connection(smtp_server, smtp_port, username, password):
  try:
    ...
    with smtplib.SMTP_SSL(smtp_server, smtp_port) as server:
      ...
      logging.info(
          f"Successfully connected to SMTP server {smtp_server} on port {smtp_port} with SSL"
      )
  except Exception as e:
    ...
    try:
      ...
      with smtplib.SMTP(smtp_server, smtp_port) as server:
        ...
        server.login(username, password)
        logging.info(
            f"Successfully connected to SMTP server {smtp_server} on port {smtp_port} with TLS"
        )
