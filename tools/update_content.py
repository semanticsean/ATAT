...
agents_json_path = os.path.join(current_script_directory, '..', 'agents', 'agents.json')

content_json_path = os.path.join(current_script_directory, '..', 'static', 'content.json')

image_static_path = os.path.join(current_script_directory, '..', 'static', 'atat.png')
...
response = openai.ChatCompletion.create(
    ...
    messages=[{
        ...
        "user",
        "content":
        f"Write an introduction for a website representing a team of AI agents working in {company_info}."
    }],
    temperature=0.7,
    max_tokens=512)
return response.choices[-1].message['content'].strip()
...
def main(social_image_url="default_social_image_url", logo_url="default_logo_url"):
...
parser = argparse.ArgumentParser(description='Update website content and metadata.')
parser.add_argument('--social_image_url', type=str, default="default_social_image_url")
